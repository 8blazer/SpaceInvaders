    ...
    void Update()
    {
        ...
        if (bulletTimer > bulletTime)
        {
            ...
            while (i < 3)
            {
                ...
                bullet.GetComponent<Rigidbody2D>().velocity = new Vector3(Random.Range(-7.0f, 8.0f), 7, 0);
        ...
        if (spawned)
        {
            if (moving)
            {
                if (moveRight)
                {
                    GetComponent<Rigidbody2D>().velocity = new Vector2(moveSpeed + ((900 - health) / 70), 0);
